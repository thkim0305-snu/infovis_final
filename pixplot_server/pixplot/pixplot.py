...
def load_and_filter_data(**kwargs):
    """Main method for filtering images given user metadata (if provided)"""

    all_loaded_data = load_input_files(**kwargs)

    # validate loaded images, remove any that are in error
    num_valid = 0
    for i in all_loaded_data:
    ...
    filtered_vecs = []
    image_names = []
    for _, d in enumerate(all_loaded_data):
        if d.valid:
            md = d.metadata
            vector = d.vec
            ...
            filtered_vecs.append(vector)
            image_names.append(md['id'])

    # save the metadata
    kwargs["metadata"] = filtered_metadata
    write_metadata(**kwargs)

    return [image_names, filtered_data, filtered_metadata, filtered_vecs]
...
def get_metadata_list(**kwargs):
    ...
    metadata_list = kwargs["metadata"].split("|")
    # for metadata_name in metadata_list:
    #     with open(metadata_name,'r') as fp:
...
def get_manifest(**kwargs):
    ...
    imagelist = {
        "cell_sizes": sizes,
        "images": [i + '.jpg' for i in kwargs["image_paths"]],
        "atlas": {
            "count": len(atlas_ids),
            "positions": pos,
        },
...
def get_atlas_data(**kwargs):
    ...
    for idx, i in enumerate(stream_images(**kwargs)):
        if isinstance(i, Image):
            ...
            cell_data = np.random.normal(0, 1, (kwargs["cell_size"],3,3))#i.resize_to_height(kwargs["cell_size"])
            _, v, _ = cell_data.shape
            appendable = False
            if (x + v) <= kwargs["atlas_size"]:
                appendable = True
...
def get_hotspots(layouts={}, use_high_dimensional_vectors=True, **kwargs):
    ...
    for idx, i in enumerate(z.labels_):
        if i != -1:
            ...
            d[i]["img"] =  kwargs["image_paths"][idx] + '.jpg'#clean_filename(kwargs["image_paths"][idx])
...
def write_images(**kwargs):
    ...
    for i in stream_images(**kwargs):
        if isinstance(i, Image):
            ...
            filename = i.metadata['id'] + '.jpg'#clean_filename(i.caption) 
            # copy original for lightbox
            out_dir = join(kwargs["out_dir"], "originals")
            if not exists(out_dir):
                os.makedirs(out_dir)
            out_path = join(out_dir, filename)
            if not os.path.exists(out_path):
                resized = np.random.normal(0, 1, (1, 1, 3))#i.resize_to_height(600)
                resized = array_to_img(resized)
                save_img(out_path, resized)
            # copy thumb for lod texture
            out_dir = join(kwargs["out_dir"], "thumbs")
            ...
            out_path = join(out_dir, filename)
            # img = array_to_img(i.resize_to_max(kwargs["lod_cell_height"]))
            img = array_to_img(np.random.normal(0, 1, (1, 1, 3)))
            save_img(out_path, img)
...
def parse():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a  numpy array file of feature vectors of the text in metadata.csv. Numpy shape is: [[num_images, size_of_embedding]]. File must be created using np.save",
        required=True,
    )
    parser.add_argument(
        "--metadata",
