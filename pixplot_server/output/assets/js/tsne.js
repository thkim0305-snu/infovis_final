...
Data.prototype.load = function () {
  ...
  get(getPath(config.data.dir + '/' + config.data.file),
    ...
    function (err) {
      console.error('ERROR: Could not load manifest file', err);
    }.bind(this)
  );
};
...
Data.prototype.parseManifest = function(json) {
  ...
  if (json.metadata) imageLabels.loadLabels();
...
Layout.prototype.set = function(layout, enableDelay) {
  ...
  // if (layout == 'download-updates-icon') return;
...
function World() {
  ...
  this.defaultLabelColor = new THREE.Color(0x00ff00);
...
World.prototype.plotPoints = function() {
  ...
  for (var drawCallIdx in drawCallToCells) {
    ...
    geometry.setAttribute('labelBorderColor', attrs.labelBorderColor);
...
World.prototype.getGroupAttributes = function(cells) {
  ...
  for (var i=0; i<cells.length; i++) {
    ...
    var labelRgb = this.defaultLabelColor;
    it.labelBorderColor[it.labelBorderColorIterator++] = labelRgb.r;
    it.labelBorderColor[it.labelBorderColorIterator++] = labelRgb.g;
    it.labelBorderColor[it.labelBorderColorIterator++] = labelRgb.b;
  ...
  var position = new THREE.BufferAttribute(positions, 3, true, 1),
      ...
      labelBorderColor = new THREE.InstancedBufferAttribute(it.labelBorderColor, 3, true, 1);
  ...
  labelBorderColor.usage = THREE.DynamicDrawUsage;
...
World.prototype.getCellIterators = function(n) {
  return {
    ...
    texIndexIterator: 0,
    labelBorderColor: new Float32Array(n * 3),
    labelBorderColorIterator: 0
  }
}
...
World.prototype.setBorderColorImages = function(indices, colors) {

  var vals = new Float32Array(data.cells.length * 3);
...
Picker.prototype.init = function() {
  ...
  world.canvas.addEventListener('mousemove', this.onMouseMove.bind(this)); // Add this line
...
// Add a hover text element to the DOM
const hoverText = document.querySelector('#tooltip');
hoverText.style.position = 'absolute';
hoverText.style.background = 'rgba(0, 0, 0, 0.8)';
...
hoverText.style.fontSize = '32px'; // Set the default font size
hoverText.style.display = 'none'; // Initially hidden

// Add mousemove event to the Picker prototype
Picker.prototype.onMouseMove = async function(e) {
  const mousePos = this.getClickOffsets(e); // Get mouse position in canvas
  const cellIdx = this.select({ x: mousePos.x, y: mousePos.y });

  if (cellIdx !== -1) {
    // Mouse is over a cell
    hoverText.style.left = `${e.clientX + 10}px`; // Offset to avoid mouse overlap
    hoverText.style.top = `${e.clientY + 10}px`;
    hoverText.style.display = 'block';
    // hoverText.textContent = `Cell Index: ${cellIdx}`; // Update the text
    // Fetch data from the JSON file associated with the cell
    try {
      
      const response = await fetch(getPath(`data/metadata/file/${cellIdx}.jpg.json`));
      if (!response.ok) throw new Error(`Failed to fetch ${cellIdx}.jpg.json`);
      const data = await response.json();

      // Display a specific key value from the JSON
      hoverText.textContent = `${data.desc}`;
    } catch (error) {
...
Modal.prototype.showCells = function(cellIndices, cellIdx) {
  ...
  function showModal(json) {
    ...
    if (filename)
      self.setLabel(filename);

    // add the meta save button handler
    var saveButton = document.querySelector('#save-labels');
    if (saveButton) {
      saveButton.addEventListener('click', self.handleSaveLabel.bind(this));
    }
  }
  // prepare the modal
  var image = new Image();
...
Modal.prototype.setLabel = function(filename) {

  fl = imageLabels.filenameLabel[filename]
  l = imageLabels.labelToId[fl]

  var labelOptions = document.querySelectorAll('input[name="label-option"]')
...
Modal.prototype.handleSaveLabel = function(e) {
  ...
  // get filename of the image we are updating the label
  var imageTitle = document.querySelector("#image-title");
  var filename = imageTitle.innerText;

  // handles label updates and redrawing
  imageLabels.updateFilenameLabel(filename, selectedId);
...
function Labels() {
  ...
  this.elems = {
    legendContainer: document.querySelector('#legend-container'),
    // downloadIcon: document.querySelector('#download-updates-icon'),
  };

  // this.elems.downloadIcon.addEventListener('click', this.downloadUpdates.bind(this));
}
...
Labels.prototype.loadLabels = function() {
  var url = config.data.dir + '/metadata/labels/id_to_label.json';
  get(getPath(url), this.handleJson.bind(this));

  var url = config.data.dir + '/metadata/labels/id_to_color.json';
  get(getPath(url), this.handleColor.bind(this));
  
  // load the image-label mapping.
  var url = config.data.dir + '/metadata/labels/filenames_label.json';
  get(getPath(url), this.handleFileLabel.bind(this));
}

Labels.prototype.downloadUpdates = function() {
  
  // loop through updated labels
  f = 'image_name, label, updated\r\n';
  for(var imageFilename in this.filenameLabel) {
    var label = this.filenameLabel[imageFilename];
    var r = imageFilename + ',' + label + ','
    if (imageFilename in this.userLabelUpdates)
      r += ' True';
    else
      r += ' False';
    r += '\r\n';
  ...
  var filename = 'updated_labels.csv';

  this.download(filename, f);
}

Labels.prototype.download = function(filename, text) {
...
Labels.prototype.createLegend = function() {
  ...
  for(var colorId in this.idToLabel) {
    var label = this.idToLabel[colorId]
    var color = this.labelColors[colorId];
    var htmlColor = this.getHtmlColor(color);

    var labelChild = document.createElement('div');
    labelChild.style = "display: flex;";

    var colorBlock = document.createElement('div');
    colorBlock.style = 'align-items: center; width: 15px; height:15px; border-radius: 3px; border: 1px solid #717171; background-color: ' + htmlColor;
    var labelSpan = document.createElement('div');
    labelSpan.style = 'display: inline-block; line-height: 20px;';
    labelSpan.innerText = label;

    labelChild.appendChild(colorBlock);
    labelChild.appendChild(labelSpan);

    legendContainer.appendChild(labelChild);
  }
}

Labels.prototype.getHtmlColor = function(color) {

  var r = (color[0] * 255).toString(16).padStart(2, '0');
  var g = (color[1] * 255).toString(16).padStart(2, '0');
  var b = (color[2] * 255).toString(16).padStart(2, '0');
  var htmlColor = '#' + r + g + b;

  return htmlColor;
}

Labels.prototype.handleColor = function(json) {
  this.labelColors = json;
}

Labels.prototype.handleJson = function(json) {
  this.idToLabel = json;
  
  for(var key in this.idToLabel) {
    val = this.idToLabel[key]
    this.labelToId[val] = key  
  }
}

Labels.prototype.handleFileLabel = function(json) {
  this.filenameLabel = json;

  // check if a update file is in session storage to allow page refreshes
  // store the changes
  var storedLabelUpdates = sessionStorage.getItem('labelChanges');
  if (storedLabelUpdates) {
    this.userLabelUpdates = JSON.parse(storedLabelUpdates);
  }
}

Labels.prototype.redrawBorders = function() {
...
Labels.prototype.updateLabels = function() {
  ...
  for(var i=0;i<data.json.images.length;i++) {
    ...
    var label = this.filenameLabel[filename];
    var labelId = this.labelToId[label];
    indices.push(i);
    colors.push(this.labelColors[labelId]);
  }

  // update the colors of image borders
  world.setBorderColorImages(indices, colors);
}
...
function Filter(obj) {
  ...
  for (var i=0; i<this.values.length; i++) {
    ...
    // hacky set color, add a function to change background color.
    var colorId = imageLabels.labelToId[this.values[i].replaceAll('__', ' ')];
    var color = imageLabels.labelColors[colorId];
    var r = (color[0] * 255).toString(16).padStart(2, '0');
    var g = (color[1] * 255).toString(16).padStart(2, '0');
    var b = (color[2] * 255).toString(16).padStart(2, '0');
    var htmlColor = '#' + r + g + b;
    child.style = 'accent-color: ' + htmlColor;
...
function Tooltip() {
  ...
  this.targets = [
    ...
    {
      elem: document.querySelector('#download-updates-icon'),
      text: 'Download label updates'
    }
  ];
...
function loadNewManifest(manifestFile) {
  // Clear existing data
  data.cells = [];
  data.textures = [];
  data.layouts = [];
  ...
  // reset label
  imageLabels.idToLabel = {};
  imageLabels.labelToId = {};
  imageLabels.filenameLabel = {};
  imageLabels.userLabelUpdates = {};

  // Reset the load button until progress reaches 100%
  const loadButton = document.getElementById("load-button");
  loadButton.disabled = true;

  // Update the config with the new manifest file
  config.data.file = manifestFile;

  // Reload data and initialize visualization
  data.load();
...
World.prototype.clearScene = function() {
  // Remove all objects from the scene group
...
Welcome.prototype.reloadWorld = function() {
  requestAnimationFrame(function() {
    world.init();
    picker.init();
    text.init();
...
document.getElementById("manifest-file").addEventListener("change", (event) => {
  const selectedFile = event.target.value; // Get the selected file
  loadNewManifest(selectedFile); // Reinitialize variables
});

document.getElementById("load-button").addEventListener("click", () => {
  world.clearScene(); // Clear the WebGL scene
  welcome.reloadWorld(); // Start the world
});
